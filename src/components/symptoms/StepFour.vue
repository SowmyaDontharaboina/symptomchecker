  ...
  <div>
    ...
    <v-stepper v-model="e6" vertical>
      ...
      <v-card-actions style="display: inherit;">
        <v-btn v-bind:class="{ selected: activeIndex === childindex }" text v-for="(answer,childindex) in healthyQuestion.answers" 
        @click="submitQuestions(index+1,childindex)" :key="childindex">
        {{ answer }}
        </v-btn>
      </v-card-actions>
        </v-card>
...
import db from '@/firebase/init';
 export default {
  ...
  created() {
    let self = this;
    db.collection('healthQuestions').get()
    .then(snapshot => {
